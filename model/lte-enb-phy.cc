...
LteEnbPhy::ReceiveLteControlMessageList (std::list<Ptr<LteControlMessage> > msgList)
{
  ...
  //msglist = lista de mensajes de control desde abajo (NPRACH, Feedbacks, DL HARQ, etc)
    {
...
LteEnbPhy::StartSubFrame (void)
{
   ...
   */
   ...
   //TBs UL esperados “hoy” → armar el receptor para Msg3
  if (m_legacy_lte){
  ...
  SendControlChannels (ctrlMsg); // la funcion antgerior primero procesa los mensajes de control para crear su pronostico de llegada y despues los transmite
